...
public class StudentManager {
	...
	public static void studentInsertManager() throws SQLException {
		...
		StudentVO svoName = new StudentVO();
		svoName.setName(name);
		System.out.println("1");
		boolean studentCheck = sDao.studentCheck(svoName);
		if (studentCheck == false) {
			System.out.println("2");
			ArrayList<StudentVO> studentNameList = new ArrayList<StudentVO>();
			studentNameList = sDao.studentNameSelect(svoName);
			printStudentList(studentNameList);

			System.out.println(svoName.getName() + "의 이름을 가진학생이 존재합니다 확인 후 입력 바랍니다 Y / N");
